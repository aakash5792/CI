...
class menucat extends CI_Controller
{
	...
	public function index(){
		$data =$this->common->index('menu_category');
	...
	public function menucat_list(){
		$sql_data =$this->common->sql_list('menu_category');
	...
	public function menucat_action(){
		$data =$this->common->index('menu_category');
		...
		if($this->validation($data,$Input_data) ) {
			if($Input_data['submit'] == 'save') {
		 	...
		 	$folder = str_replace(' ','_', trim(strtolower($Input_data['menu_category_name'])));
			
			if (!is_dir('./application/controllers/'.$folder)) {
			    mkdir('./application/controllers/'.$folder, 0777);
			    mkdir('./application/models/'.$folder, 0777);
			    mkdir('./application/views/'.$folder, 0777);
			     mkdir('./assets/app/js/'.$folder, 0777);
			     $file_name = $folder.'_function.js';
				write_file('./assets/app/js/'.$folder.'/'.$file_name, '');
			}
	...
	public function menucat_delete(){
		$data =$this->common->index('menu_category');
